...
Production qualification on 2026-09-10 completed that separate evidence. OAuth admitted only the configured owner for authenticated tool discovery and PRJ-0008 access; unauthenticated MCP remained denied. Transaction `TXN-CTCANARY-MTVXZHZD` committed synthetic task `TASK-CTCANARYMTVXZHZD` from revision 2 to 3, exact replay returned the identical result, changed-payload reuse was rejected, and the canonical state plus both generated human views converged to revision 3.

On 2026-09-11 the operator authorized full convergence repair for every active registry project. Continuity remains data-preserving: the writer never rewinds a revision or projection, archived projects remain excluded, canonical business changes still require typed transactions and committed receipts, and a missing human notification channel does not suppress durable incidents or retry evidence.

Receipt recovery initially failed because the production Project Guard still ran source `c9c9fde`, before the internal read-only receipt route, while the Control Tower already expected that route. Project Guard version `792dc59d-e8b3-4d06-90dc-009aa130b104` corrected the deployment skew with exact tag `git-ed572801a9a123a9efcd390e1be4241d3f9e5d5d`; a repeated OAuth lookup returned the committed revision-3 receipt. The deployment preserved continuity `stable`, MutationGate `enforce`, and the sole convergence writer `PRJ-0008: repair`. Its creation at `2026-09-10T19:57:58Z` restarts the 24-hour canary gate, so PRJ-0003 remains ineligible before `2026-09-11T19:57:58Z` even if every other proof stays green.

The route-only rollback exercise made only the Control Tower unavailable with HTTP 503 while Project Guard health stayed HTTP 200. Restoration selected pre-rollback version `e64cd336-a267-4c0c-89df-4418bdd11778` at 100%; OAuth metadata, anonymous denial, authenticated PRJ-0008 revision-3 reading, and Project Guard health all recovered. This releases the optional governed channel for existing projects but does not switch the stable continuity path. PRJ-0007 still requires exact Founder acceptance of its C0 document before any durable write, and PRJ-0003 remains excluded until the independent 24-hour convergence gate and its other qualification evidence pass.

...
The synthetic transaction `TXN-CTCANARY-MTVXZHZD` created `TASK-CTCANARYMTVXZHZD` on `PRJ-0008`. Its canonical receipt is `committed`, from revision 2 to revision 3 with event `EVT-000003`. An exact replay returned byte-identical tool output, while reuse of the same transaction ID with changed payload bytes was rejected. The canonical `state.json`, generated `STATE.md`, and generated `HANDOFF.md` all subsequently reached revision 3; no direct canonical file edit was used.

The first post-rollback receipt lookup exposed a deployment-order defect: the Control Tower contained the new read tool while Project Guard was still on pre-route source `c9c9fde26daf7ea82db2503acfc7b52c573e3c47`, so the internal `/receipt` request returned `not_found`. The merged guard was then deployed as version `792dc59d-e8b3-4d06-90dc-009aa130b104`, tagged `git-ed572801a9a123a9efcd390e1be4241d3f9e5d5d`, with continuity `stable`, MutationGate `enforce`, and only `PRJ-0008: repair` retained. Health proved the exact Git SHA and version, and the same OAuth receipt lookup then returned the committed transaction at revision 3. This new guard deployment restarts the isolated 24-hour clock at `2026-09-10T19:57:58Z`; PRJ-0003 cannot be released before `2026-09-11T19:57:58Z` and all other gates remain mandatory.

Rollback qualification deployed deny-only version `82037a09-221c-4095-9c44-3c99a5862c60` at 100%, where `/mcp` returned HTTP 503 while `project-os-guard` health remained HTTP 200. It then restored the exact pre-rollback operational version `e64cd336-a267-4c0c-89df-4418bdd11778` at 100%. After propagation, OAuth metadata returned HTTP 200, anonymous MCP returned HTTP 401, Project Guard health remained HTTP 200, and an authenticated read-only PRJ-0008 context returned revision 3. The restored version is Cloudflare's secret-change derivative of the reviewed Control Tower deployment; its only secrets are the GitHub client secret and OAuth cookie key.

The governed channel is therefore qualified for existing projects. `PRJ-0007` has not been mutated because no exact C0 document has yet been explicitly accepted for `working.write`. `PRJ-0003` remains read-only until its separate canary clock and every independent convergence gate pass; this Control Tower qualification does not shorten that gate.

...
The first PR-head CI run (`36015661958`, SHA `370b45f`) failed in `test/execution-guard.spec.ts:714`: the test's global Dropbox mock counted 34 calls while the bounded finalization runtime allowed at most 32. The locally reproduced failure clarified two sources of overcount: unrelated guard alarms share that mock, and the `beforeHttp` callback originally incremented its counter for the attempted 33rd call that it rejected *before* HTTP. The fixture now records only calls permitted by that runtime and separately checks zero reuse calls; it does not weaken the production budget. The exact suite passed 38/38 targeted tests after the correction, and an independent reviewer checked the counter against the provider hook and budget. A sixth complete suite exited 0: **257/257 files, 1,631/1,631 tests** in 189.90 s; typecheck and diff check passed. New PR-head CI remains required before E13.

...
**Spec:** [Spécification acceptée](../specs/2026-09-07-post-commit-convergence-reliability-design.md), commit `97684d3e164d690bd7e54cf1aa50be41cab6cf10`, base runtime `696714deea50e83d6c459bc8901e0d6408841eb3`.
...
Ce document est le seul fichier modifié par la mission de planification. Les cases concernent une exécution ultérieure ; aucune case ne constitue une autorisation de canary, déploiement, fusion ou mutation canonique. Ne pas refaire le design. Le système forme une chaîne de sûreté commune : un plan unique, découpé en lots, évite que journaux, fencing et admission divergent entre sous-projets.
...
Les cas de panne d’une ligne de matrice deviennent autant de cycles indépendants, pas une étape monolithique. Aucune implémentation future n’est lancée pendant la rédaction.
...
**Files:** Modify `docs/deployment.md`, `docs/materialization.md`, `docs/fault-injection.md`, `docs/continuity.md` pour joindre les résultats de qualification de la future implémentation. Test : suites existantes et nouvelles. Aucun changement de spec acceptée. Aucun changement de config production dans cette tâche.
...
- [ ] **4. Vérifier les sept phases du rollout**, sans les lancer dans cette mission :
...
La mission de rédaction s’arrête au commit/push de ce document et à la confirmation de la PR draft #148. Les deux modes d’exécution du skill (subagent-driven-development ou executing-plans) sont réservés à une instruction ultérieure d’implémentation ; aucune exécution runtime n’est engagée par ce plan.

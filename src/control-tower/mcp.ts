...
async function submitGuarded(env: { PROJECT_GUARD: DurableObjectNamespace; REGISTRY_GUARD: DurableObjectNamespace }, projectId: string, kind: "transaction" | "document", request: Record<string, unknown>) {
  ...
  const path = kind === "transaction" ? (request.operation === "project.create" ? "/create" : "/transaction") : "/document";

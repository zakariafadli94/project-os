...
export class ManagedDocumentService {
  ...
  private publishedHeadFromEvidence(evidence: {
    ...
    destination: { path: string; object_id: string; revision_token: string; size: number; integrity_hash: { algorithm: string; value: string }; provider_id: string };
  }): ManagedDocumentHead {
...
function receiptFor(requestId: string, record: DocumentVersionRecord): ManagedDocumentReceipt {
  ...
  return {
    ...
    ...(record.source_candidate_request_id
      ? { candidate_request_id: record.source_candidate_request_id, accepted: true as const, published: record.stage === "published" ? true as const : undefined }
      : {})
  };
}

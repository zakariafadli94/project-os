...
describe("convergence slice budget", () => {
  ...
  it("exposes the same hard call budget to the Dropbox client", () => {
    ...
    expect(scope.deadlineMs).toBe(22_000);
  });
  ...
  it("stops provider work before the deadline reserved for the durable checkpoint", () => {
    ...
    expect(providerRequestScopeFor(budget).deadlineMs).toBe(22_000);
    expect(providerReservedEffectScopeFor(budget).deadlineMs).toBe(25_000);
    expect(providerCheckpointScopeFor(budget).deadlineMs).toBe(25_000);
    now = 22_000;
    expect(budget.canStartEffect(1)).toBe(false);
  });
});

...
describe("WorkspaceProjectionWriter", () => {
  ...
  it("rejects a removed deliverable recreated before final publication", async () => {
    const objects = new InstrumentedObjects();
    const writer = new WorkspaceProjectionWriter(objects, 1);
    const path = "/workspace/DELIVERABLES/DEL-3301.md";
    objects.files.set(path, `${MANAGED_NOTICE}\nrecreated after deletion`);
    const removed: ProjectionOutputEvidence = {
      relative_path: "DELIVERABLES/DEL-3301.md",
      input_hash: await sha256Text("legacy-input"),
      content_hash: await sha256Text(`${MANAGED_NOTICE}\nold generated bytes`),
      source_revision: 3
    };

    await expect(writer.verifyAbsentOutputs(new Map([["deliverable:DEL-3301", removed]]), "/workspace"))
      .rejects.toBeInstanceOf(MaterializationOutputConflictError);
  });

  it("verifies removed outputs with bounded parallel reads", async () => {
    const objects = new InstrumentedObjects();
    objects.readDelay = 10;
    const writer = new WorkspaceProjectionWriter(objects, 2);
    const removed = new Map(await Promise.all([1, 2, 3, 4].map(async (index) => [
      `deliverable:DEL-33${index}`,
      {
        relative_path: `DELIVERABLES/DEL-33${index}.md`,
        input_hash: await sha256Text(`legacy-input-${index}`),
        content_hash: await sha256Text(`legacy-content-${index}`),
        source_revision: 3
      } satisfies ProjectionOutputEvidence
    ] as const)));

    await writer.verifyAbsentOutputs(removed, "/workspace");

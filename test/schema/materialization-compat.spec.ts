...
describe("ProjectState schema evolution vs materialization", () => {
  it("produces identical current projection semantics from V1 or V2 nested state", async () => {
    sequence = 0;
    ...
    expect(planV1.projection_version).toBe(CURRENT_PROJECTION_VERSION);
    expect(planV2.projection_version).toBe(CURRENT_PROJECTION_VERSION);
  ...
  it("carries unaffected outputs forward with their older source revision when the next commit carries ProjectState V2", async () => {
    ...
    expect(nextPlan.projection_version).toBe(CURRENT_PROJECTION_VERSION);
  ...
  it("does not change projection semantics merely because the canonical state encoder advances", async () => {
    ...
    expect(plan.projection_version).toBe(CURRENT_PROJECTION_VERSION);

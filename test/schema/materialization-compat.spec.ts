...
describe("ProjectState schema evolution vs materialization", () => {
  it("keeps the current projection at v5 and produces identical projection semantics from V1 or V2 nested state", async () => {
    sequence = 0;
    ...
    expect(planV1.projection_version).toBe(5);
    expect(planV2.projection_version).toBe(5);
  ...
  it("carries unaffected outputs forward with their older source revision when the next commit carries ProjectState V2", async () => {
    ...
    expect(nextPlan.projection_version).toBe(5);
  ...
  it("does not change projection semantics merely because the canonical state encoder advances", async () => {
    ...
    expect(plan.projection_version).toBe(5);
